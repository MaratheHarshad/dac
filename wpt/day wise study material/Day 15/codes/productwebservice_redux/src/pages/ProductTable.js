...
import { setproddetails } from "../redux/action/productaction";
export default function ProductTable() {
  //create a array as state
  //const [prodarr,setprodarr]=useState([]);
  let prodarr = useSelector((state) => state.allprod.proddata);

  const [searcharr, setsearcharr] = useState([]);
  //let searcharr=useSelector((state)=>state.allprod.proddata)
  ...
  const fetchdata = () => {
    ProductService.getAllProducts()
      .then((result) => {
        ...
        let action1 = setproddetails(result.data);
        dispatch(action1);
      })
      .catch();
  };
  ...
  return (
    <div>
      ...
      <Link to="/form">
        <button type="button" name="btn" id="add" className="btn btn-info">
          Add new Product
        </button>
      </Link>
      &nbsp;&nbsp;&nbsp;&nbsp;
